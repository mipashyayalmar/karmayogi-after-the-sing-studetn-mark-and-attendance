...
def student_profile(request, student_id=None):
    ...
    if user_profile.employee_type != 'student':
        return redirect('permission_denied')  # Redirect if not a student

    # Check if the student's profile is complete
    if not user_profile.is_complete:  # Assuming `is_complete` is a field in UserProfile
        return redirect('create_profile')  # Redirect to profile creation page

    # Retrieve the correct student profile
    if not student_id:
        try:
            student = AcademicInfo.objects.get(userprofile=user_profile)
        except AcademicInfo.DoesNotExist:
            return redirect('create_profile')  # Redirect if no academic info exists
    else:
    ...
    if student.userprofile != user_profile:
        return redirect('permission_denied')  # Redirect for invalid access
