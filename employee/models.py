...
class EmployeeJobInfo(models.Model):
    ...
    def __str__(self):
        return f"{self.department.name}" if self.department else "No Department"
...
class EmployeeMessage(models.Model):
    employee = models.ForeignKey(PersonalInfo, on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"Message to {self.employee.name} from {self.sender.user.username}"
